...
import { getFetch, modalThongBao, postData } from "./helper.js";
...
async function postImage(image) {
  const data = await postData("../ajax/taiKhoan.php", {
    action: "anhDaiDien",
    anhdaidien: image,
  });
  return data;
       ...
       <h3>Tài khoản</h3>
        <div class="content__inner chitiet">
          ...
          <div  class="inputInfo--flat mt-1">
            ...
            <input type="password" class="inputLargeX" name = "MK" id ="MK" value ="12345678" disabled/> 
          </div>
          <div  class="inputInfo--flat mt-1">
          <label class="labelLarge">Ảnh đại diện</label>
          <input type="file" class="inputLargeX" style="height: 3rem;" name = "anhdaidien" id ="AnhDaiDien" value ="" / disabled> 
          </div>
        </div>
...
function init() {
  ...
  btnSua.addEventListener("click", async (e) => {
    ...
    anhdaidien.removeAttribute("disabled");
    ...
    btnXacNhan.addEventListener("click", async (e) => {
      ...
      const anhdaidien = file;
      if (anhdaidien) {
        let rest = await postImage(anhdaidien);
        if (rest) {
      ...
      if (res) {
        await dangXuat();
        await modalThongBao("Cập nh~", true);
        window.location.href("../index.html");
      }
    });
  });
